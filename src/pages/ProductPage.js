...
const CourseContent = ({ content }) => {
  if (!content || !content.course_content || content.course_content.length === 0) {
    return null;
  }

  const courseContentList = content.course_content.map((item, index) => (
...
const ProductPage = ({ course }) => {
  ...
  return (
    <div>
      ...
      <div>
        Created by {' '}
        <a href={`/instructors/${slugify(instructor)}`} style={{ textDecoration: 'underline', color: 'blue' }}>
          {instructor}
        </a>
      </div>
